...
describe("InMemorySearchableRepository Unit Tests", () => {
    ...
    describe("search method", () => {
        it("should apply only paginate when other params are null", async () => {
            ...
            const search = new search_params_1.SearchParams();
            const result = await repository.search(search);
            expect(result).toStrictEqual(new search_result_1.SearchResult({
        ...
        it("should apply paginate and filter", async () => {
            ...
            repository.items = items;
            let result = await repository.search(new search_params_1.SearchParams({ page: 1, per_page: 2, filter: "TEST" }));
            expect(result).toStrictEqual(new search_result_1.SearchResult({
            ...
            result = await repository.search(new search_params_1.SearchParams({ page: 2, per_page: 2, filter: "TEST" }));
            expect(result).toStrictEqual(new search_result_1.SearchResult({
                items: [items[3]],
                total: 3,
                current_page: 2,
        ...
        describe("should apply paginate and sort", () => {
            ...
            const arrange = [
                {
                    search_params: new search_params_1.SearchParams({
                        page: 1,
                        per_page: 2,
                        sort: "name",
                    }),
                    search_result: new search_result_1.SearchResult({
        ...
        it("should search using filter, sort and paginate", async () => {
            ...
            const arrange = [
                {
                    params: new search_params_1.SearchParams({
                        page: 1,
                        per_page: 2,
                        sort: "name",
                        filter: "TEST",
